...
def parse_arguments():
    parser = aurlrgparse.ArgumentParser(description="Download data from OONI and Radar APIs and store it in MongoDB.")
...
def download_ooni_web_connectivity(cc, start_date, end_date):
    ...
    try:
        ...
        response.raise_for_status()
        df = pd.read_csv(StringIO(response.text))
        insert_dataframe_into_mongodb(df, f'ooni_web_collection')
        logger.info(f"Inserted ooni web data into MongoDB for {cc}")
        
        upload_to_minio(df.to_csv(), f'ooni_web_collection.csv')
...
def download_ooni_circumvention_tools(cc, start_date, end_date):
    ...
    try:
        ...
        response.raise_for_status()
        df = pd.read_csv(StringIO(response.text))
        insert_dataframe_into_mongodb(df, f'ooni_circumvention_collection')
        logger.info(f"Inserted ooni circumvention data into MongoDB for {cc}")
        
        upload_to_minio(df.to_csv(), f'ooni_circumvention_collection.csv')
    ...
    try:
        ...
        if database == 'radar_ip':
            for cc in filtered_cc:
                download_radar_dataset('ip_version', cc, start_date, end_date)
                logger.info(f"Downloading radar dataset (IP version) for {cc} done")

        elif database == 'radar_tls':
            for cc in filtered_cc:
                download_radar_dataset('tls_version', cc, start_date, end_date)
